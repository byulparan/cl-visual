...
(defmethod ns:mouse-wheel ((view visual-canvas) event location-x location-y)
  ...
  (when (use-mouse view)
    (let* ((x (float (ns:objc event "deltaX" :double) 1.0))
	   ...
	   (limit .04)
	   (gain .05))
      (if (> limit (abs x)) (setf x 0.0))
      (if (> limit (abs y)) (setf y 0.0))
      (cond ((ns:ctrl-p event) (gfx:track-mouse-zoom camera  (- x) (- y) gain))
    	    ((ns:shift-p event) (gfx:track-mouse-pan camera (- x) y gain))
    	    (t (gfx:track-mouse-spin camera (- x) y gain))))))
